...
function onInit()
    ...
    OptionsManager.registerOption2("FTARTD", false, "option_header_automation", "option_label_FTARTD",
        "option_entry_cycler",
        { labels = "option_val_pconly|option_val_npconly|option_val_off", values = "pconly|npconly|off", baselabel =
        "option_val_all", baseval = "all", default = "all" })
...
function testTraitResolutionSubHandler(rRoll, vRollResult, rSource, vTargets, rContext)
    ...
    if vRollResult.nTotalScore >= 4 and not vRollResult.bCriticalFailure then
        sendMessage = {
            ["type"] = OOB_MESSAGE_TYPE_TEST_INITIATION_SUCCESS,
...
function testDefenceResolutionSubHandler(rRoll, rRollResult, rSource, vTargets, rContext)
    ...
    local sendMessage = {
        ["type"] = OOB_MESSAGE_TYPE_TEST_DEFENCE_ROLL_COMPLETED,
...
function onTestDefenceRollCompleted(data)
    ...
    local nodePendingTest = DB.findNode(data.sPendingTest)
    if nodePendingTest then
        ...
        local bFirstRoll = (DB.getValue(nodePendingTest, "defencerolled", 0) == 0)
        if (data.bCritFail == "true" or ((not bOldCritFail) and nOldResult < tonumber(data.nDefenceResult)) or (bFirstRoll)) then
        ...
        FastTests.updateTestResult(nodePendingTest);
    end
end
...
function notifyUsersOfNewTest(nodePendingTest)
    Comm.deliverOOBMessage({
        ["type"] = OOB_MESSAGE_TYPE_TEST_NEW_PENDING_TEST,
...
function onCreativeCombatTableRolled(rRoll, rSource, rTargets)
    ...
    local rOOBMessage = {
        ["type"] = OOB_MESSAGE_TYPE_TEST_CCTABLE_ROLLED,
...
function sendSecondWindMessage(nodePendingTest)
    ...
    rMessage = {}
    rMessage.text = string.format("Second Wind: %s may remove a level of Fatigue or a Wound (their choice).",
        sAgressorName)
...
function sendSeizeTheMomentMessage(nodePendingTest)
    ...
    "Seize the Moment: After %s resolves this turn, they immediately get another entire turn (including movement). They may use the turn to go on hold.",
        sAgressorName)
    rMessage.sender = "FastTests"
...
function sendInsightMessage(nodePendingTest)
    ...
    rMessage = {}
    rMessage.text = string.format(
    "Insight: %s has new Insight into %s's nature. Once during this encounter, they may add +d6 to any Trait roll made to directly attack, affect or damage %s.",
        sAgressorName, sVictimName, sVictimName)
    rMessage.sender = "FastTests"
...
function sendSetbackMessage(nodePendingTest)
    ...
    rMessage = {}
    rMessage.text = string.format(
    "Setback: %s suffers a setback of some sort. She might fall off a ledge, lose the confidence of her minions (who then desert her), take a rash but foolish action, or simply lose her next turn as she attempts to recover from whatever situation she finds herself in.",
        sVictimName)
    rMessage.sender = "FastTests"
...
function requestCCRoll(nodePendingTest)
    ...
    if sAgressorType == "pc" then
        ...
        if not bOwnerPresent then
            ...
            local rOOBMessage = {
                ["type"] = OOB_MESSAGE_TYPE_TEST_CCROLL_REQUEST,
...
function updateTestResult(nodePendingTest)
    local nAttackScore = DB.getValue(nodePendingTest, "targetvalue",4)
    local nDefenceScore = DB.getValue(nodePendingTest, "defencescore",0)
    local nResult = nAttackScore - nDefenceScore
    if nResult >= 4 then
            ...
            if (not sCCResultKeyword) or sCCResultKeyword=="" then
                if Session.IsHost or Session.IsLocal then
                    if DB.getValue(nodePendingTest, "ccrollrequested", 0) == 0 then
                        DB.setValue(nodePendingTest,"ccrollrequested","number",1)
                        FastTests.requestCCRoll(nodePendingTest)
                    end
                end
            end
        end
